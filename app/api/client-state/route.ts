...
// GET /api/client-state?since=<rev> → { success, data: { rev, keys } }
// Returns every key whose per-key rev is greater than `since`
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ success: true, data: { rev: store.rev, keys } }, { headers: NO_STORE });
}
