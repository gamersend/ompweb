...
test("deleting a local bookmark propagates and survives a stale redelivery (wave 3 P2)", async () => {
  const fake = fakeStorage({
  ...
  const fetch = fakeFetch((call) => {
    ...
    if (call.method === "DELETE") {
      return jsonResponse(200, { success: true, data: { rev: 50, tombstone: { rev: 50, itemId: call.body.itemId, deletedAt: 1000 }, idempotent: false } });
    }
  ...
  try {
    ...
    const puts = fetch.calls.filter((call) => call.method === "PUT");
    assert.equal(puts.length, 1);
    assert.deepEqual(puts[0].body.value, [{ entryId: "kept", ts: 1 }], "the deletion-shaped value reaches the server");
    const deletes = fetch.calls.filter((call) => call.method === "DELETE");
    assert.equal(deletes.length, 1, "the delete marker is pushed");
    assert.equal(deletes[0].body.itemId, "doomed");
    await pullClientStateSyncForTests(); // the pre-deletion copy (rev 2) is redelivered…
    assert.equal(isBookmarked("s1", "doomed"), false, "…but the tombstone filters it: no resurrection");
    await flushClientStateSyncForTests();
...
test("clearBookmarks empties the local list and tombstones it against the next pull", async () => {
  const fake = fakeStorage();
  let gets = 0;
  ...
  try {
    ...
    await flushClientStateSyncForTests();
    assert.ok(fetch.calls.some((call) => call.method === "DELETE"), "the clear became delete markers");
    await pullClientStateSyncForTests(); // since=1; key rev bumped to 2 → redelivered
    assert.equal(isBookmarked("s1", "e1"), false, "the tombstone keeps the clear in force");
  } finally {
    disposeSyncForTests();
    setBookmarksStorage(null);
  }
