 ...
 * `connecting` or `live` is a no-op transition (same phase back).
 */

export type LivePhase = "idle" | "connecting" | "live" | "failed" | "ended";
...
export function reduceLiveEvent(state: LiveState, event: LiveEvent): LiveState {
  switch (event.kind) {
    case "start":
      // One live session at a time: a start while connecting or live is refused.
