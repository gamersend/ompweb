...
test.beforeEach(() => {
  ...
  // Deterministic external-client source (see runs-board.test.mjs): the test
  // machine's own live omp clients must not leak into frames/assertions.
...
test("GET /api/runs serves the aggregator snapshot inside the API envelope", async () => {
  ...
  assert.deepEqual(Object.keys(body.data).sort(), ["externalClients", "revision", "runs", "watchers"]);
  assert.deepEqual(body.data.runs, []);
  assert.deepEqual(body.data.externalClients, []);
...
test("events route: an external-client-only change still emits a frame", { timeout: 8000 }, async () => {
  const alpha = { pid: 4242, clientId: "4242-uuid", projectDir: "C:/repo/terminal", startedAt: "2026-09-23T00:00:00.000Z" };
  let clients = [];
  board.setExternalClientsReaderForTests(() => ({ supported: true, clients }));

  // No sessions at all: the running set never changes, so the ONLY reason a
  // frame can appear is the external client set moving.
  const ac = new AbortController();
  ...
  try {
    ...
    assert.deepEqual(connect.runs, []);
    assert.deepEqual(connect.externalClients, []);

    clients = [alpha];
    await board.pollBoardOnce();
    const frame = JSON.parse((await stream.waitFor((chunk) => chunk.includes('"type":"runs"'), 4000)).text.trim().split("\n\n")[0].slice(6));
    assert.equal(frame.type, "runs");
    assert.deepEqual(frame.runs, [], "no session row changed");
    assert.deepEqual(frame.externalClients.map((client) => client.pid), [4242]);
    assert.ok(frame.revision > connect.revision, "the external change bumped the revision");
  } finally {
    ac.abort();
    stream.stop();
  }
