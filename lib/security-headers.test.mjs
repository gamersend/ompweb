...
for (const phase of [PHASE_PRODUCTION_BUILD, PHASE_DEVELOPMENT_SERVER]) {
  ...
  test(`service worker + manifest PWA headers (${phase})`, async () => {
    // A cached SW script can pin a stale app for weeks; it must revalidate,
    // and Service-Worker-Allowed widens its control to the whole origin.
    const sw = await effectiveHeaders(phase, "/sw.js");
    assert.equal(sw.get("service-worker-allowed"), "/");
    assert.equal(sw.get("cache-control"), "no-cache");
    const manifest = await effectiveHeaders(phase, "/manifest.webmanifest");
    // Without the explicit type some static servers guess text/plain, which
    // breaks installability; the short cache keeps installs fresh without
    // hammering the server.
    assert.equal(manifest.get("content-type"), "application/manifest+json");
    assert.match(manifest.get("cache-control") ?? "", /max-age=3600/);
    // The handler-agnostic protections still ride along.
    assert.equal(sw.get("x-content-type-options"), "nosniff");
    assert.equal(manifest.get("x-content-type-options"), "nosniff");
  });
}
