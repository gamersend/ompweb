...
test("preserves display aliases, sort order, and launch config", () => {
  ...
  const withLaunchConfig = updateProjectsPresentation(updated, [{ path: P("/one"), launchConfig: { profile: "work", advisor: true, extraArgs: ["--verbose"] } }]);
  assert.deepEqual(withLaunchConfig.projects[0].launchConfig, { profile: "work", advisor: true, extraArgs: ["--verbose"] });
});
test("strips reserved and flag-smuggled launch args from disk configs", () => {
  assert.equal(isReservedLaunchArg("--cwd"), true);
  assert.equal(isReservedLaunchArg("--cwd=/evil"), true);
  assert.equal(isReservedLaunchArg("--mode=json"), true);
  assert.equal(isReservedLaunchArg("--resume=/tmp/x.jsonl"), true);
  assert.equal(isReservedLaunchArg("rpc-ui"), true);
  assert.equal(isReservedLaunchArg("--verbose"), false);
  const registry = parseProjectRegistry(JSON.stringify({ version: 1, projects: [{
    path: "/one", addedAt: "2026-01-01T00:00:00.000Z", hidden: false,
    launchConfig: { profile: "--evil", advisor: true, extraArgs: ["--verbose", "--cwd=/evil", "--resume"] },
  }] }));
  // Dash-leading profile and reserved args are dropped; advisor survives.
  assert.deepEqual(registry.projects[0].launchConfig, { profile: undefined, advisor: true, extraArgs: ["--verbose"] });
});
test("re-adding a workspace without config preserves its launch config", () => {
  const seeded = parseProjectRegistry(JSON.stringify({ version: 1, projects: [{
    path: P("/one"), addedAt: "2026-01-01T00:00:00.000Z", hidden: true,
    launchConfig: { profile: "work" },
  }] }));
  const revived = upsertProject(seeded, P("/one"), "2026-02-01T00:00:00.000Z", undefined);
  assert.deepEqual(revived.projects[0].launchConfig, { profile: "work", advisor: undefined, extraArgs: undefined });
  const replaced = upsertProject(seeded, P("/one"), "2026-02-01T00:00:00.000Z", { advisor: true });
  assert.deepEqual(replaced.projects[0].launchConfig, { advisor: true });
});
...
// ── Phase 3 (quick-launch): registry v2 + launch-field guards ──────────────

test("migrate v1 → v2 preserves every field and bumps only the version", () => {
  const parsed = parseProjectRegistry(JSON.stringify({
    version: 1,
    projects: [{
      path: "/one", addedAt: "2026-01-01T00:00:00.000Z", hidden: false,
      alias: "One", sortOrder: 3,
      launchConfig: {
        profile: "work", advisor: true, extraArgs: ["--verbose"],
        prompt: "do things", model: "anthropic:claude-x", thinkingLevel: "high", toolsPreset: "default",
      },
    }],
  }));
  assert.equal(parsed.version, 2, "reading migrates the file to the current version");
  const entry = parsed.projects[0];
  assert.equal(entry.path, P("/one"));
  assert.equal(entry.addedAt, "2026-01-01T00:00:00.000Z");
  assert.equal(entry.alias, "One");
  assert.equal(entry.sortOrder, 3);
  assert.deepEqual(entry.launchConfig, {
    profile: "work", advisor: true, extraArgs: ["--verbose"],
    prompt: "do things", model: "anthropic:claude-x", thinkingLevel: "high", toolsPreset: "default",
  });

  // migrateRegistry is the pure seam: a v1-shaped file → v2, fields intact
  // (verbatim — migration never rewrites paths, only parse canonicalizes).
  const migrated = migrateRegistry({ version: 1, projects: [{ path: "/x", addedAt: "2026-01-01T00:00:00.000Z", hidden: true }] });
  assert.equal(migrated.version, 2);
  assert.equal(migrated.projects[0].hidden, true);
  assert.equal(migrated.projects[0].path, "/x");
});

test("parse guards drop invalid v2 launch fields and keep the rest", () => {
  const registry = parseProjectRegistry(JSON.stringify({
    version: 1,
    projects: [{
      path: "/one", addedAt: "2026-01-01T00:00:00.000Z", hidden: false,
      launchConfig: {
        profile: "work",
        prompt: "x".repeat(4097), // oversized → dropped (cap is 4096)
        model: "no-colon", // unparseable "provider:modelId" ref → dropped
        thinkingLevel: "ultra", // not on the ladder → dropped
        toolsPreset: "everything", // not a preset → dropped
      },
    }],
  }));
  assert.deepEqual(registry.projects[0].launchConfig, { profile: "work", advisor: undefined, extraArgs: undefined }, "invalid values dropped, valid ones preserved");

  const good = parseProjectRegistry(JSON.stringify({
    version: 1,
    projects: [{ path: "/two", launchConfig: { prompt: "hi", model: "p:m", thinkingLevel: "off", toolsPreset: "none" } }],
  }));
  assert.deepEqual(
    good.projects[0].launchConfig,
    { profile: undefined, advisor: undefined, extraArgs: undefined, prompt: "hi", model: "p:m", thinkingLevel: "off", toolsPreset: "none" },
  );

  // A config carrying ONLY invalid v2 fields parses to nothing.
  const empty = parseProjectRegistry(JSON.stringify({
    version: 1,
    projects: [{ path: "/three", launchConfig: { prompt: 42, model: ":x", thinkingLevel: "nope", toolsPreset: "all" } }],
  }));
  assert.equal(empty.projects[0].launchConfig, undefined);
});
